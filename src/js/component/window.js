 ...
 */
define(['jquery', 'text!../template/components.rac', 'ui'], function ($, template, UI) {
  UI.Window = UI.extend(UI, function (options) {
    ...
    if (typeof options === 'string') {
      ...
      if (this.type === 'success') {
        ...
        this.iconClass = 'icon-undo pull-left';
      }
    ...
    this.show = function () {
      ...
      popup.find("#message_title").css({
        "display": "block",
        "font-size": options.msgTitleFontSize || "24px",
        //"padding-left": "30px",
        "line-height": "1.5em"
      });
      // 如果是单行，即只有msgtitle没有message 的话， 调整其行高
      if (!this.message) {
        popup.find("#message_title").css({
          "line-height": "40px"
        });
        popup.find('.ui-popup-content').addClass('mt-20 mb-20');
      }
      ...
      if (this.notShowMsgTitle === true) popup.find("#message").css("margin", "0");  // 如果不显示 msgTitle 和 icon 的话 message 不需要 margin属性
