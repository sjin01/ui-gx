    ...
    UI.Panel = UI.extend(UI,function(options){
        ...
        var inputEvt = function(event){
            ...
            if( !target.data("isPanelASelect") ){
                if(!panelB){
                    appendPanelBToDom(value, false, event);
                }
        ...
        var initPanel = function(_options){
            var event = window.event || _options.event;
            var styleStr = "";
            if(event){
                var target = event.target;
                var _offsetLeft = target.offsetLeft;
                var _offsetTop = target.offsetTop;
                var _offsetWidth = target.offsetWidth;
                var _offsetHeight = target.offsetHeight;
                var indexSearchBoxEle = target.parentNode.parentNode;
                var _isbOffsetLeft = indexSearchBoxEle.offsetLeft;
                var _isbOffsetTop = indexSearchBoxEle.offsetTop;
                var _left = 0;
                var _top = 0;
                if("panelA" == _options.panelName){
                    var triangleIconOffsetLeft = 38;
                    var triangleIconOffsetWidth = 8;
                    _left = _isbOffsetLeft + _offsetLeft + _offsetWidth / 2 - triangleIconOffsetLeft;
                    _top = _isbOffsetTop + _offsetTop + _offsetHeight + triangleIconOffsetWidth;
                }else{
                    _left = _isbOffsetLeft + _offsetLeft;
                    _top = _isbOffsetTop + _offsetTop + _offsetHeight + 1;
                }
                styleStr = "left:" + _left + "px;top:" + _top + "px;";
            }
        ...
        var appendPanelAToDom = function(_queryParamValue, isShow, event){
            panelA = initPanel({
                panelName : "panelA",
                el : me.el,
        ...
        var appendPanelBToDom = function(_queryParamValue, isShow, event){
            panelB = initPanel({
                panelName : "panelB",
                el : me.el,
